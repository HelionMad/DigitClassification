  ...
  """
    ...
    def trainAndTune(self, trainingData, trainingLabels, validationData, validationLabels, kgrid):
        ...
        for label in self.legalLabels:
            self.prior_p[label] = trainingLabels.count(label) / float(len(trainingLabels))
                ...
                for feature in self.features:
                    ...
                    for datum in trainingData:
                        if trainingLabels[i] == label:
                            time = time + datum[feature]
                            #print "datum[feature]",datum[feature]
                            sum = sum + datum.totalCount()
                        i = i + 1

                    prob = float(time + k) / (sum + k * n_feature)
        ...
        for k in kgrid:
            ...
            for guess in guesses:
                if guess == validationLabels[i]:
                    correct = correct+1
                i = i + 1
            accuracy = correct / float(n_image)
    ...
    def calculateLogJointProbabilities(self, datum):
        ...
        for label in self.legalLabels:
            ...
            logJoint[label] = math.log(prior_p) + sum
        return logJoint
